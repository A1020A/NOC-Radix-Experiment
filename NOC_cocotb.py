...
class NOC_Net_Tester:
    def __init__(self, RADIX_IN, RADIX_OUT, ADDR_WIDTH, DATA_WIDTH, NETWORK_DEPTH, logging_task):
        self.RADIX_IN = RADIX_IN
        self.RADIX_OUT = RADIX_OUT
        self.FINAL_OUT = RADIX_OUT**(NETWORK_DEPTH+1)
        self.ADDR_WIDTH = ADDR_WIDTH
        self.DATA_WIDTH = DATA_WIDTH
        self.NETWORK_DEPTH = NETWORK_DEPTH
        self.logging_task = logging_task
        self.packet_len = self.ADDR_WIDTH + self.DATA_WIDTH
        self.packets = [[] for _ in range(RADIX_OUT**(NETWORK_DEPTH+1))] # keeps track of packets at each output node

    def send(self, FIFO_FULL, src=None, dests=None, payload=None):
        while (1):
            if src is None:
                src = random.randrange(0, self.RADIX_IN)
            else:
                ...
                # print(f"FIFO_FULL:{FIFO_FULL}")
            if dests is None:
                dests = [random.randrange(0, self.RADIX_OUT) for _ in range(self.NETWORK_DEPTH+1)]
            if payload is None:
                payload = random.randrange(0, 2**self.DATA_WIDTH)
            if (FIFO_FULL[self.RADIX_IN-1-src] == "0"):
                break
        ...
        addr_eff=round(math.log2(self.RADIX_OUT))
        packet = BinaryValue(dests[0], n_bits=addr_eff, bigEndian=False).binstr + BinaryValue(dests[1], n_bits=addr_eff, bigEndian=False).binstr  + ("0"*(self.ADDR_WIDTH - 2*addr_eff)) + BinaryValue(payload, n_bits=self.DATA_WIDTH, bigEndian=False).binstr
        # print(packet)
        r = 1
        dest_flatten = self._calc_Net_dest(dests)
        
        self.packets[dest_flatten].append(packet)
        data_vec = ""
        for i in range(self.RADIX_IN):
            if (i != src):
                data_vec = ("0"*self.packet_len) + data_vec
                ...
                data_vec = packet + data_vec
        enq_vec =  BinaryValue(1 << src, n_bits=self.RADIX_IN, bigEndian=False)
        self.logging_task.log.debug(f"sending from src:{src} through dest1:{dests[0]} to dest2:{dests[1]}")
        self.logging_task.log.debug(f"pkt logged to final dest:{dest_flatten}")
        self.logging_task.log.debug(f"sending pkt: {f'0x{int(packet, 2):12x}':^12}")
        return enq_vec, LogicArray(data_vec)
    

    def receive(self, FIFO_ENQ_downstream, data_vec):
        if (FIFO_ENQ_downstream != BinaryValue(0, n_bits=self.FINAL_OUT, bigEndian=False)):
            self.logging_task.log.debug(f"FIFO_ENQ_downstream:{FIFO_ENQ_downstream.value.binstr}")
            for idx, i in enumerate(FIFO_ENQ_downstream.value.binstr):
                # print(f"idx:{idx} i:{i}")
                if (i == "1"):
                    received_pkt = data_vec.value.binstr[idx*self.packet_len:idx*self.packet_len+self.packet_len]
                    self.logging_task.log.debug(f"receiving packet from dest:{self.FINAL_OUT-1-idx}, pkt: {f'0x{int(received_pkt, 2):12x}':^12}")
                    self.debug_print_wait_buffer(self.FINAL_OUT-1-idx)
                    assert(data_vec.value.binstr[idx*self.packet_len:idx*self.packet_len+self.packet_len] in self.packets[self.FINAL_OUT-1-idx])
                    self.packets[self.FINAL_OUT-1-idx].remove(data_vec.value.binstr[idx*self.packet_len:idx*self.packet_len+self.packet_len])
                    self.logging_task.log.debug("success")
        pass
    ...
    # assume the same NOC accross all layers
    def _calc_Net_dest(self, dests):
        res = 0
        power = len(dests)
        for i in dests:
            power -= 1
            res += i*(self.RADIX_OUT**(power))
        return res
...
# @cocotb.test()
async def fully_random_NET(dut):
    # seed = random.randrange(sys.maxsize)
    # rng = random.Random(seed)
    # print(f"seed:{seed}")
    # random.seed(5592969042884215283)
    ...
    Tester = NOC_Net_Tester(int(args['RADIX_IN']), int(args['RADIX_OUT']), int(args['ADDR_WIDTH']), int(args['DATA_WIDTH']), int(args['NETWORK_DEPTH']), task_clk)
    task_clk.log.debug("Logging Started")
    dut.FIFO_FULL_downstream.value = BinaryValue("1"*Tester.RADIX_OUT, n_bits=Tester.RADIX_OUT**(Tester.NETWORK_DEPTH+1), bigEndian=False)
    dut.rst_l.value = 0
    for _ in range(3):
        await RisingEdge(dut.clk)
    dut.rst_l.value = 1
    ...
    for _ in range(100):
        ...
        dut.FIFO_IN.value = data_vec_final
        task_clk.log.debug(f"enq_vec:{enq_vec_final.binstr}")
        if (dut.FIFO_FULL_downstream.value == BinaryValue(0, n_bits=Tester.RADIX_OUT**(Tester.NETWORK_DEPTH+1), bigEndian=False)):
            Tester.receive(dut.FIFO_ENQ_downstream, dut.FIFO_OUT)

        if (random.randint(0, 10) < 8):
            dut.FIFO_FULL_downstream.value = BinaryValue(0, n_bits=Tester.RADIX_OUT**(Tester.NETWORK_DEPTH+1), bigEndian=False)
        else:
            dut.FIFO_FULL_downstream.value = BinaryValue("1"*Tester.RADIX_OUT, n_bits=Tester.RADIX_OUT**(Tester.NETWORK_DEPTH+1), bigEndian=False)
